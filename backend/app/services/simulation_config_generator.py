...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        time_config_result = self._generate_time_config(
            ...
            wait_callback=self._create_wait_callback(progress_callback, 1, total_steps, "正在生成时间配置")
        )
        ...
        event_config_result = self._generate_event_config(
            ...
            wait_callback=self._create_wait_callback(progress_callback, 2, total_steps, "正在生成事件配置")
        )
        ...
        for batch_idx in range(num_batches):
            ...
            batch_configs = self._generate_agent_configs_batch(
                ...
                start_idx=start_idx,
                simulation_requirement=simulation_requirement,
                wait_callback=self._create_wait_callback(
                    progress_callback, 3 + batch_idx, total_steps, 
                    f"正在生成Agent配置 ({start_idx + 1}-{end_idx}/{len(entities)})"
                )
            )
    ...
    def _create_wait_callback(
        ...
        def wait_cb(remaining, total):
            progress_callback(
                current_step, 
                total_steps, 
                f"{stage_message} (LLM Rate Limit Cooldown: {remaining}s)"
            )
        return wait_cb
    ...
    def _call_llm_with_retry(
        ...
        """带重试的LLM调用，包含JSON修复逻辑和模型fallback"""
        import re
        import time
        ...
        try:
            ...
            logger.error(f"LLM生成配置失败 (使用LLMClient已尝试多次): {str(e)}")
            raise e
...
- reasoning (string): 简要说明为什么这样配置"""
        ...
        try:
            return self._call_llm_with_retry(prompt, system_prompt, wait_callback=wait_callback)
        except Exception as e:
...
{{
    "agent_configs": [
        ...
        try:
            result = self._call_llm_with_retry(prompt, system_prompt, wait_callback=wait_callback)
