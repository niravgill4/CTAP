...
class OasisAgentProfile:
    ...
    # 临床/经济状态字段 (CTAP 专属)
    income_tier: Optional[str] = None          # Low, Medium, High
    digital_literacy: Optional[str] = None      # Low, High
    transit_accessibility: Optional[str] = None # Low, High
    mobility_score: Optional[int] = None        # 1-10
    caregiver_support: Optional[str] = None     # Yes, No
    state: str = "active"                       # active, dropped (模拟状态)
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        # 添加临床/经济状态字段
        if self.income_tier:
            profile["income_tier"] = self.income_tier
        if self.digital_literacy:
            profile["digital_literacy"] = self.digital_literacy
        if self.transit_accessibility:
            profile["transit_accessibility"] = self.transit_accessibility
        if self.mobility_score is not None:
            profile["mobility_score"] = self.mobility_score
        if self.caregiver_support:
            profile["caregiver_support"] = self.caregiver_support
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "caregiver_support": self.caregiver_support,
            "state": self.state,
...
class OasisProfileGenerator:
    ...
    INDIVIDUAL_ENTITY_TYPES = [
        ...
        "patient", "participant", "subject", "volunteer", "investigator", "physician", "doctor"
    ]
    ...
    GROUP_ENTITY_TYPES = [
        ...
        "hospital", "clinic", "researchcenter", "clinicalsite"
    ]

    # 非模拟类型实体（临床元数据，必须过滤掉，不能生成Agent）
    NON_SIMULATABLE_ENTITY_TYPES = [
        "medication", "drug", "drugdose", "treatment", "procedure",
        "sideeffect", "adverseevent", "endpoint", "criteria", "protocol",
        "symptom", "disease", "condition", "measurement", "labresult"
    ]
    ...
    def __init__(
        ...
        fallback_str = getattr(Config, 'LLM_FALLBACK_MODELS', '')
    ...
    def generate_profile_from_entity(
        ...
        return OasisAgentProfile(
            ...
            income_tier=profile_data.get("income_tier") or entity.attributes.get("income_tier"),
            digital_literacy=profile_data.get("digital_literacy") or entity.attributes.get("digital_literacy"),
            transit_accessibility=profile_data.get("transit_accessibility") or entity.attributes.get("transit_accessibility"),
            mobility_score=profile_data.get("mobility_score") or entity.attributes.get("mobility_score"),
            caregiver_support=profile_data.get("caregiver_support") or entity.attributes.get("caregiver_support"),
            state="active",
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        # Zep has a hard limit of 400 characters for the query string
        safe_entity_name = entity_name[:350] if entity_name else ""
        comprehensive_query = f"关于{safe_entity_name}的所有信息、活动、事件、关系和背景"
    ...
    def _generate_profile_with_llm(
        ...
        try:
            ...
            logger.error(f"LLM生成人设失败 (使用LLMClient已尝试多次): {str(e)}")
            last_error = e
...
9. income_tier: 经济水平，必须是英文: "Low", "Medium", "High"
10. digital_literacy: 数字素养，必须是英文: "Low", "High"
11. transit_accessibility: 交通便利度，必须是英文: "Low", "High"
12. mobility_score: 行动力评分，1-10的整数
13. caregiver_support: 是否有照顾者支持，必须是英文: "Yes", "No"
...
- 机构账号发言要符合其身份定位"""
    ...
    def generate_profiles_from_entities(
        ...
        # 过滤非模拟类型的实体 (0.5mg, Nausea 等不能变成 Agent)
        original_count = len(entities)
        filtered_entities = []
        for e in entities:
            etype = (e.get_entity_type() or "Entity").lower()
            if etype in self.NON_SIMULATABLE_ENTITY_TYPES:
                logger.debug(f"跳过非模拟实体: {e.name} ({etype})")
                continue
            
            # 名字检查 (如果是纯数字+单位，通常也是误提取的元数据)
            import re
            if re.match(r'^\d+\.?\d*\s*(mg|ml|kg|g|%)', e.name.lower()):
                logger.debug(f"跳过数值型实体: {e.name}")
                continue
        ...
        if total == 0:
            logger.warning(f"过滤后没有可模拟的实体 (原始数量: {original_count})")
            return []
