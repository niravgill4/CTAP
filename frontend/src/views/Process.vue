<template>
  <div class="process-page">
    <!-- 顶部导航栏 -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">⬡ CTAP</div>
      
      <!-- Step Indicator -->
      <div class="nav-center">
        <div class="step-badge">STEP 01</div>
        <div class="step-name">Knowledge Map Construction</div>
      </div>
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">Loading knowledge graph data...</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">Graph construction will begin automatically once complete</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">Data will appear momentarily...</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        ...
        <div class="process-content">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  Uploads your clinical trial documents and uses an LLM to extract a domain-specific ontology: entity types (Patient, ClinicalVisit, AdverseEvent) and relationships (DROPS_OUT_OF, CAUSES_FATIGUE, REQUIRES_FASTING)
                </div>
              </div>
              
              <!-- Ontology Generation进度 -->
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">Extraction Progress</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">Extracted Entity Types ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">Extracted Relationship Types ({{ projectData.ontology.relation_types?.length || 0 }})</div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">Awaiting ontology extraction...</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">What this step does</div>
                <div class="detail-content">
                  Chunks the uploaded PDFs and calls the Zep graph API to build a clinical knowledge graph — extracting patients, visits, adverse events, and relationships
                </div>
              </div>
              
              <!-- Waiting本体完成 -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Awaiting ontology extraction to complete...</div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">Map Complete</div>
                <div class="phase-api">Ready for patient agent spawning</div>
              </div>
        ...
        <div class="project-panel">
          <div class="project-header">
            ...
            <span class="project-title">TRIAL PROJECT INFO</span>
          </div>
          <div class="project-details" v-if="projectData">
            ...
            <div class="project-item">
              <span class="item-label">Trial Objective</span>
...
const currentPhase = ref(-1) // -1: 上传中, 0: Generating Ontology, 1: GraphRAG Build, 2: 完成
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return 'Building Map'
...
const goToNextStep = () => {
  ...
  alert('Environment setup in dev...')
}
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = '没有待上传的文件，请Home重新操作'
    loading.value = false
    return
  }
  ...
  try {
    ...
    error.value = 'Project Init Failed: ' + (err.message || 'UnknownError')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    error.value = 'Load Project Failed: ' + (err.message || 'UnknownError')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: 'Starting map build...'
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = 'Map build started...'
      
      // 保存 task_id 用于轮询
      const taskId = response.data.task_id
      
      // 启动Graph数据轮询（独立于任务状态轮询）
      startGraphPolling()
      
      // 启动任务状态轮询
      startPollingTask(taskId)
    } else {
      error.value = response.error || '启动GraphBuild Failed'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = '启动GraphBuild Failed: ' + (err.message || 'UnknownError')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ GraphRAG Build完成，正在加载完整数据...')
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'GraphBuild Failed: ' + (task.error || 'UnknownError')
        buildProgress.value = null
      }
    }
  } catch (err) {
