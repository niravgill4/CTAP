    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          addLog(`Close模拟环境失败: ${closeRes.error || 'UnknownError'}`)
          // 如果优雅Close失败，尝试强制停止
...
const forceStopSimulation = async () => {
  try {
    ...
    if (stopRes.success) {
      ...
      addLog(`强制停止模拟失败: ${stopRes.error || 'UnknownError'}`)
    }
  } catch (err) {
...
const loadSimulationData = async () => {
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      addLog(`加载模拟数据失败: ${simRes.error || 'UnknownError'}`)
    }
  } catch (err) {
...
onMounted(async () => {
  ...
  // 检查并Close正在运行的模拟（用户从 Step 3 返回时）
