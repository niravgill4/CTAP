    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Starting...' : 'Generate Attrition Report' }} 
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div v-if="isPaused" class="pause-banner">
        ...
        <div class="pause-info">
          ...
          <div class="pause-hint">Auto-retry in progress... checking every 30 seconds</div>
        </div>
        <button class="pause-retry-btn" @click="handleRetry">↻ Retry Now</button>
      </div>
      <!-- Timeline Header -->
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- 通用回退：Unknown类型或有 content 但未被上述处理 -->
...
const actionIds = ref(new Set()) // For deduplication
...
const resetAllState = () => {
  ...
  stopHealthCheck()  // Stop health-check polling
}
...
const doStartSimulation = async () => {
  ...
  // Reset all state to avoid contamination from previous simulation
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('Launching patient swarm simulation (dual-platform)...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog('✓ Patient swarm engine started')
...
// Health-check: pings the API every 30s when paused, auto-resumes when credits are back
const startHealthCheckPolling = () => {
  stopHealthCheck()
  addLog('Starting API health-check (every 30s)...')
  healthCheckTimer = setInterval(async () => {
    try {
      const res = await getRunStatus(props.simulationId)
      if (res.success && res.data) {
        const data = res.data
        const hasError = data.runner_status === 'error' || data.runner_status === 'paused'
        const errorMsg = (data.error || data.message || '').toLowerCase()
        const isCreditError = errorMsg.includes('429') || errorMsg.includes('quota') || 
                             errorMsg.includes('rate_limit') || errorMsg.includes('insufficient')
        
        if (!hasError && !isCreditError) {
          // Credits are back! Auto-resume
          addLog('✓ API credits restored — auto-resuming simulation')
          stopHealthCheck()
          isPaused.value = false
          pauseReason.value = ''
          doStartSimulation()
        } else {
          addLog('⏳ API still unavailable, retrying in 30s...')
        }
      }
    } catch (err) {
      addLog('⏳ Health-check failed, retrying in 30s...')
    }
  }, 30000)
}
...
const handleRetry = () => {
  addLog('Manual retry triggered by user')
  stopHealthCheck()
  isPaused.value = false
  pauseReason.value = ''
  doStartSimulation()
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 检测模拟是否Complete（通过 runner_status 或平台完成状态判断）
      ...
      if (isCompleted || platformsCompleted) {
        ...
        addLog('✓ Simulation complete — patient attrition data ready')
        phase.value = 2
        stopPolling()
        emit('update-status', 'completed')
      }
    ...
    // Detect API credit / rate-limit errors
    if (res.success && res.data) {
      const data = res.data
      const errorMsg = (data.error || data.message || '').toLowerCase()
      const isCreditError = errorMsg.includes('429') || errorMsg.includes('quota') || 
                           errorMsg.includes('rate_limit') || errorMsg.includes('insufficient') ||
                           data.runner_status === 'error'
      
      if (isCreditError && !isPaused.value) {
        isPaused.value = true
        pauseReason.value = data.error || data.message || 'API rate limit or credit quota reached. The simulation will auto-resume when credits are restored.'
        addLog(`⚠ Simulation paused: ${pauseReason.value}`)
        stopPolling()
    ...
    // Network errors or API completely down — also trigger pause
    const errMsg = (err.message || '').toLowerCase()
    if ((errMsg.includes('429') || errMsg.includes('quota') || errMsg.includes('rate')) && !isPaused.value) {
      isPaused.value = true
      pauseReason.value = 'API rate limit reached. Will auto-retry when credits are restored.'
      addLog(`⚠ Simulation paused: ${pauseReason.value}`)
      stopPolling()
      startHealthCheckPolling()
    } else {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Report generation already in progress, please wait...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('Launching attrition report generation...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Attrition report task started: ${reportId}`)
...
onMounted(() => {
  addLog('Patient trial simulation engine initializing...')
  if (props.simulationId) {
    doStartSimulation()
  }
})
