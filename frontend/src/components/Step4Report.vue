      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Open Master Dashboard</span>
          ...
          <div v-if="hasError && !isComplete" class="report-error-banner">
            <div class="error-icon">⚠</div>
            <div class="error-info">
              <div class="error-title">Report Generation Paused</div>
              <div class="error-reason">{{ errorMessage }}</div>
              <div class="error-hint">You can retry now or wait for API credits to replenish.</div>
            </div>
            <button class="error-retry-btn" :disabled="isRetrying" @click="retryGeneration">
              <span v-if="isRetrying">Retrying...</span>
              <span v-else>↻ Retry Generation</span>
            </button>
          </div>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Content generated，但整个章节可能还没完成) -->
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // 如果任务Complete，滚动到顶部；否则滚动到底部跟随最新日志
            if (isComplete.value) {
    ...
    if (errMsg.includes('429') || errMsg.includes('quota') || errMsg.includes('rate')) {
      hasError.value = true
      errorMessage.value = 'API rate limit reached during report generation. Click Retry when credits are restored.'
      stopPolling()
    }
  }
}
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        // Detect API errors in console output
        for (const log of newLogs) {
          const logText = (log || '').toLowerCase()
          if ((logText.includes('429') || logText.includes('quota') || logText.includes('rate_limit') || logText.includes('insufficient_quota')) && !hasError.value) {
            hasError.value = true
            errorMessage.value = 'API rate limit or quota exceeded during report generation. Click Retry when your credits are restored.'
            stopPolling()
            break
          }
        }
...
// Retry report generation after API error
const retryGeneration = async () => {
  if (!props.simulationId) {
    addLog('Cannot retry: missing simulationId')
    return
  }
  ...
  errorMessage.value = ''
  addLog('Retrying report generation...')
  ...
  try {
    ...
    if (res.success && res.data) {
      addLog(`✓ Report regeneration started: ${res.data.report_id}`)
      // Reset state and restart polling
      startPolling()
    } else {
      hasError.value = true
      errorMessage.value = res.error || 'Retry failed. Please try again later.'
      addLog(`✗ Retry failed: ${res.error || 'Unknown error'}`)
    }
  } catch (err) {
    hasError.value = true
    errorMessage.value = err.message || 'Retry failed due to network error.'
...
.error-info { flex: 1; }
.error-title { font-size: 14px; font-weight: 700; color: #92400E; margin-bottom: 4px; }
.error-reason { font-size: 12px; color: #78350F; line-height: 1.4; }
.error-hint { font-size: 11px; color: #A16207; margin-top: 4px; font-style: italic; }
...
.error-retry-btn:hover:not(:disabled) { background: #333; }
