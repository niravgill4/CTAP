      ...
      >
        ...
        <div class="card-header">
          ...
          <div class="card-status-icons">
            <button 
              ...
              @click.stop="handleDeleteSimulation(project.simulation_id)"
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-empty" v-else>
            ...
            <span class="empty-file-text">No files uploaded</span>
          </div>
        </div>

        <!-- 卡片标题（使用Trial Objective的前20字作为标题） -->
        <h3 class="card-title">{{ getSimulationTitle(project) }}</h3>

        <!-- 卡片描述（Trial Objective完整展示） -->
        <p class="card-desc">{{ truncateText(cleanObjective(project.simulation_requirement), 80) }}</p>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                <div class="modal-label">Trial Objective</div>
                <div class="modal-requirement">{{ selectedProject.simulation_requirement || 'No objective specified' }}</div>
              </div>

              <!-- 文件列表 -->
              <div class="modal-section">
                <div class="modal-label">Uploaded Documents</div>
                ...
                <div class="modal-empty" v-else>No documents uploaded</div>
              </div>
            </div>
            ...
            <div class="modal-actions">
              ...
              >
                ...
                <span class="btn-text">Knowledge Map</span>
              </button>
              <button 
              ...
              >
                ...
                <span class="btn-icon">◆</span>
                <span class="btn-text">Attrition Report</span>
              </button>
            </div>
            <!-- 不可回放Notice -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 (Live Simulation) and Step5 (Agent Interview) require an active session and do not support historical playback</span>
            </div>
          </div>
        </div>
      </Transition>
...
const isFetching = ref(false)     // 加载锁
let observer = null
...
// Extract a meaningful trial name for the card title
const getSimulationTitle = (project) => {
  // Strategy 1: Extract drug/trial name from uploaded filenames
  if (project.files && project.files.length > 0) {
    for (const file of project.files) {
      const name = file.filename || ''
      // Match common patterns: "Wegovy_SoA.txt", "NCT04280705_IB.txt", etc.
      const nctMatch = name.match(/NCT\d+/i)
      if (nctMatch) {
        // Try to extract drug name from filename like "NCT03548935_Wegovy_Semaglutide_STEP1_Protocol.txt"
        const parts = name.replace(/\.(txt|pdf|md|doc|docx)$/i, '').split(/[_\-]/)
        const drugParts = parts.filter(p => !p.match(/^(NCT\d+|Protocol|SoA|IB|ICF|txt)$/i) && p.length > 2)
        if (drugParts.length > 0) {
          return drugParts.slice(0, 2).join(' ') + ' Trial'
        }
        return nctMatch[0] + ' Trial'
      }
      // Match drug names directly: "Wegovy_IB.txt" → "Wegovy"
      const drugMatch = name.replace(/\.(txt|pdf|md|doc|docx)$/i, '').split(/[_\-]/)
        .filter(p => !['IB', 'ICF', 'SoA', 'Protocol', 'CTP'].includes(p) && p.length > 2)
      if (drugMatch.length > 0) {
        return drugMatch[0] + ' Trial'
      }
    }
  }
  // Strategy 2: Strip generic prefix from objective text
  const req = project.simulation_requirement || ''
  const cleaned = req.replace(/^\[?CLINICAL TRIAL ATTRITION SIMULATION\]?\s*/i, '').trim()
  if (cleaned.length > 0) {
...
// Clean the objective text for the description line
const cleanObjective = (text) => {
  if (!text) return ''
  return text.replace(/^\[?CLINICAL TRIAL ATTRITION SIMULATION\]?\s*/i, '').trim() || text
}
...
// 加载历史项目
const loadHistory = async (source = 'unknown') => {
  ...
  try {
    ...
    console.error('[HistoryDatabase] 加载历史项目失败:', error)
    projects.value = []
  } finally {
    loading.value = false
...
const handleClearAll = async () => {
  if (!confirm('Are you sure you want to clear ALL simulation history? This will delete all generated profiles, configs, and results. This action is IRREVERSIBLE.')) {
    return
  }
  ...
  // 逐个删除（后端目前没有批处理删除，为了保险我们逐个调用）
  const deletePromises = projects.value.map(async (p) => {
    try {
      const response = await deleteSimulation(p.simulation_id)
      if (response.success) {
        successCount++
      } else {
        failCount++
  ...
  if (failCount > 0) {
    alert(`Cleared ${successCount} simulations, but ${failCount} failed.`)
  }
...
onMounted(async () => {
  // 确保 DOM 渲染完成后再加载数据
  await nextTick()
  loadHistory('onMounted')
  
  // Waiting DOM 渲染后初始化观察器
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
/* 不同功能的颜色 */
.status-icon:nth-child(1).available { color: #3B82F6; } /* GraphRAG Build - 蓝色 */
...
/* 进度状态颜色 */
.card-progress.completed { color: #10B981; }    /* Complete - 绿色 */
